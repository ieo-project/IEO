...
library(GOstats)
#DEGs <- rownames(tt.filt)
guniverse <- rownames(se)
params <- new("GOHyperGParams", geneIds=DEgenes, universeGeneIds=guniverse,annotation="org.Hs.eg.db", ontology="BP",pvalueCutoff=0.05, testDirection="over")
...
# 575 genes DE (after filtering by p-value and LOG-FC) but only 501 which have a GO term
```
```{r sessioninfo}
...
plotGS(se.filt4, genesGS1, "type", main = names(rnkGS)[1], cex.lab = 2, las = 1)
plotGS(se.filt4, genesGS2, "type", main = names(rnkGS)[2], cex.lab = 2, las = 1)
