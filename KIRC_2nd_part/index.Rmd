...
table(paired_mask)
paired_se <- se.filt[,paired_mask]
dge <- dge.filt[, paired_mask]
...
#portion <- "01R"
the_mask <-  substr(colnames(dge.filt),22,25) %in% plate_list& substr(colnames(dge.filt),14,16) %in% samplevial_list
...
outcome <- paste(substr(colnames(se.filt), 9, 12), as.character(se.filt3$type), sep="-")
