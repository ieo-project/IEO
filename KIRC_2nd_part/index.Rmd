...
#se.filt4 <- se.filt3[,grep("5546|5591|4698", colnames(se.filt3), invert=TRUE)]       OLD FILTERING
#dge.filt4 <- dge.filt3[,grep("5546|5591|4698", colnames(dge.filt3), invert=TRUE)]    OLD FILTERING
...
# Calculating the number of over/underexpressed genes, and its p-values, logFC... 
...
# Y esto qué es, porque no da el mismo mumero de DE genes que en las tablas de antes...
mod0 <- model.matrix(~ 1, colData(se.filt4))
pv <- f.pvalue(assays(se.filt4)$logCPM, mod, mod0)
sum(p.adjust(pv, method="fdr")< 0.01)
hist(pv, main="",las=1)
#9825 10149 10623 10708
```
