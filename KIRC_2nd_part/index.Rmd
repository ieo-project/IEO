...
PAIRED DATAAAAAAAAAAA

```{r}
normal_IDs <- substr(colnames(se[,se$type=="normal"]),1,12)
str(normal_IDs)
paired_mask <- substr(colnames(se),1,12) %in% normal_IDs
table(paired_mask)
paired_se <- se[,paired_mask]
...
```{r DGE, include=TRUE}
dge <- DGEList(counts=assays(paired_se)$counts, genes=mcols(paired_se))
...
```{r CPM}
assays(paired_se)$logCPM <- cpm(dge, log=TRUE, prior.count=0.5)
...
barplot(dge$sample$lib.size[ord]/1e+06, las = 1, ylab = "Millions of reads", xlab = "Samples", col = c("red", "blue")[paired_se$gender[ord]], space=c(0,0))
legend("topleft", c("female", "male", "NA"), fill = c("red", "blue", "white"), inset = 0.02)
abline(h=(sum(colSums(assays(paired_se)$counts))/1e6)/ncol(paired_se))
```
...
```{r exp_filt_samples}
cpm_cutoff <- 21
mask <- colSums(assays(paired_se)$counts)/1e6 > cpm_cutoff
se.filt <- paired_se[, mask]
dge.filt <- dge[, mask]
dim(se.filt)
```
...
NO HAY QUE ELIMINAR SAMPLES --> PRIMERO MIRAMOS SI HAY BATCH EFFECT EN CADA VARIABLE Y SI LO HAY PUES CORREGIMOS POR ESA VARIABLE PERO NO ELIMINAMOS SAMPLES!
...
outcome <- paste(substr(colnames(se.filt2), 9, 12), as.character(se.filt2$type), sep="-")
names(outcome) <- colnames(se.filt2)
...
```{r hiercClustFilt}
HAY QUE ELIMINAR ESTOS SEGURO, NO?
...
tumor_se <- se.filt2[,se.filt2$type == "tumor"]
tumor_dge <- dge.filt2[,se.filt2$type == "tumor"]

## Gender
se_gender <- se.filt2[,!is.na(se.filt2$gender)]
dge_gender <- dge.filt2[,!is.na(se.filt2$gender)]
...
sel <- which(tt$logFC<=-3) # or whatever you want to use
points(tt[sel,"logFC"], -log(tt[sel,"P.value"]),col="red")
