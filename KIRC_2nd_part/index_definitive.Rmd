...
barplot(dge.filt2$sample$lib.size[ord]/1e+06, las = 1, ylab = "Millions of reads", xlab = "Samples", col = c("red", "blue")[se.filt2$type[ord]], space=c(0,0))
legend("topleft", c("tumor", "normal"), fill = c("red", "blue"), inset = 0.02)
...
```{r DEgenes}
#mnb <- rowRanges(se.filt2)$symbol == "AMPK"
#se.filt2[mnb,]
#mean(assays(se.filt2)$logCPM["7428",])
#mask <- Im[,"72"] == 1
#Im[mask,"72"]
```
...
Im <- Im[, colnames(Im) %in% rownames(DEgenes)]
#dim(Im)
...
tGSgenes <- DEgenes[match(colnames(Im), rownames(DEgenes)), "t"]
#length(tGSgenes)
#head(tGSgenes)
...
# Number of pathways: 207 (gene set size of 5) or 381 (gene set size of 3)
length(zS) 
# Number of pathways significantly enriched (p-value < 0.05): 187 (gene set size of 5) or 354 (gene set size of 3)
sum(pv_zs < 0.05) 

# FDR multiple test correction 
pvadj_zs <- p.adjust(pv_zs, method = "fdr")
# Number of pathways significantly enriched (FDR < 0.01): 187 (gene set size of 5) or 354 (gene set size of 3)
sum(pvadj_zs < 0.01)
...
DEgs_z<- merge(rnkGSzs,rnkGSpz,by="row.names")
colnames(DEgs_z) <- c("pathways","z-score","Z_p-value")
head(DEgs_z[order(DEgs_z$`z-score`,decreasing=TRUE),],10)

#write.csv(as.data.frame(DEgs), file = "GSEA_Zscore_5size.csv")
```
...
In the `c2BroadSets` object there are 833 pathways (from KEGG, BIOCARTA and REACTOME). Depending on the gene set size threshold used we remove more or less pathways:
  ...
  -Gene set minimum size of 5 genes --> 207 pathways left, from wich 187 were significantly enriched (FDR < 0.01)
 
  -Gene set minimum size of 3 genes --> 381 pathways left, from wich 353 were significantly enriched (FDR < 0.01)
...
genesGS1 <- colnames(Im)[which(Im[names(rnkGSz)[3], ] == 1)]
genesGS2 <- colnames(Im)[which(Im[names(rnkGSz)[8], ] == 1)]
genesGS3 <- colnames(Im)[which(Im[names(rnkGSz)[9], ] == 1)]
plotGS(se.filt4, genesGS1, "type", main = names(rnkGSz)[3], cex.lab = 2, las = 1)
plotGS(se.filt4, genesGS2, "type", main = names(rnkGSz)[8], cex.lab = 2, las = 1)
plotGS(se.filt4, genesGS3, "type", main = names(rnkGSz)[9], cex.lab = 2, las = 1)
...
## Change in scale detection (X2-score)

The previous GSEA analysis using Z-score detects shifts in mean expression. However, if half of the genes are up-regulated and the other half down-regulated, the mean shift cancels out. This is what is known as a change in scale. To detect such a change we should rather use the `X2-score`.

We created a data frame containing all pathways with the Z-score and $X^2$-score (and its associated p-values). The interpretation is in the report.
...
#X2-score calculation
xS <- applyByCategory(tGSgenes, Im, function(x) (sum((x - mean(x))^2) - (length(x) - 1))/(2 *(length(x) - 1)))

# Sort the gene sets by X2-score
rnkGSx <- sort(abs(xS), decreasing = TRUE)
...
# Number of pathways: 207 (gene set size of 5) or 381 (gene set size of 3)
length(rnkGSx)
# Number of pathways significantly enriched (p-value < 0.05): 207 (gene set size of 5) or 381 (gene set size of 3)
sum(pv_xs < 0.05)

# FDR multiple test correction 
pvadj_xs <- p.adjust(pv_xs, method = "fdr")
# Number of pathways significantly enriched (FDR < 0.01): 207 (gene set size of 5) or 381 (gene set size of 3)
sum(pvadj_xs < 0.01)
...
DEgs_x<- merge(rnkGSx,rnkGSpx,by="row.names")
colnames(DEgs_x) <- c("pathways","X2-score","X2_p-value")

# Merge both X2-score/p-value and Z-score/p-value into one
all_DEgs <- merge(DEgs_z,DEgs_x)
head(all_DEgs[order(all_DEgs$`X2-score`,decreasing=TRUE),],10)

par(mfrow = c(1, 3), mar = c(4, 5, 3, 4))
genesGS1 <- colnames(Im)[which(Im[names(rnkGSx)[1], ] == 1)]
genesGS2 <- colnames(Im)[which(Im[names(rnkGSx)[45], ] == 1)]
genesGS3 <- colnames(Im)[which(Im[names(rnkGSx)[36], ] == 1)]
plotGS(se.filt4, genesGS1, "type", main = names(rnkGSx)[1], cex.lab = 2, las = 1)
plotGS(se.filt4, genesGS2, "type", main = names(rnkGSx)[45], cex.lab = 2, las = 1)
plotGS(se.filt4, genesGS3, "type", main = names(rnkGSx)[36], cex.lab = 2, las = 1)

# For loop to plot all interesting pathways
#for (i in pathways){
  #topgsgenes <- colnames(Im)[which(Im[names(rnkGSx)[i], ] == 1)]
  #plotGS(se.filt4, topgsgenes, "type", main = names(rnkGSx)[i], cex.lab = 2, las = 1)
#}

#write.csv(as.data.frame(DEgs_xS), file = "GSEA_Xscore_5size.csv")
#write.csv(as.data.frame(all_DEgs), file = "GSEA_ALL_5size.csv")
```

These scatter plots (figure \@ref(fig:scatterPlotX)) show how the pathways with high X2-score have half genes upregulated and half genes downregulated.
...
# VEGF PATHWAY
#VEGF_genes <- colnames(Im)[which(Im[names(rnkGSz)[33], ] == 1)]
#plotGS(se.filt4, VEGF_genes, "type", main = names(rnkGSz)[33], cex.lab = 2, las = 1)
tt[VEGF_genes,]

# CELL CYCLE
#CC_genes <- colnames(Im)[which(Im[names(rnkGSz)[49], ] == 1)]
