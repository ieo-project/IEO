...
###GENDER
batch <- as.integer(factor(se.filt4$gender))
outcome <- paste(factor(se.filt4$gender), as.character(se.filt4$type), sep="-")
plotMDS(dge.filt4, col = batch,labels = outcome, cex = 0.7)
legend("topleft", paste("Batch", sort(unique(batch)), levels(factor(se.filt4$gender))), fill=sort(unique(batch)), inset=0.005)
...
legend("topleft", paste("Batch", sort(unique(batch)), levels(factor(tumor_stage_se$ajcc_pathologic_tumor_stage))), fill=sort(unique(batch)), inset=0.005)
```
