...
To perform quality assessment and normalization we make us of the [edgeR](http://bioconductor.org/packages/edgeR) R/Bioconductor package and we create a `DGEList` object.

```{r DGE object creation}
dge <- DGEList(counts=assays(se)$counts, genes=mcols(se))
# Saving the DGEList object into a "results" folder
...
summary(dge$sample$lib.size/1e6)
```
...
cutoff <- 21
mask <- colSums(assays(se)$counts)/1e6 > cutoff
se.filt <- se[, mask]
dge.filt <- dge[, mask]
dim(se.filt)
```

We store un-normalized versions of the data filtered by sample depth (cutoff of 21M).
...
```{r Filtering of lowly-expressed genes}
...
### 2.8. Clustering according to batch effect (hierarchical clustering)
...
### 2.8. Clustering according to batch effect (multidimensional plot)
...
head(data.frame(Log2FC = round(log2fc[ranking], digits = 3), FC = round(2^log2fc[ranking], 
    ...
    check.names = FALSE), n = 100)

## Tumor stage

tumorExp <- rowMeans(logCPM[, tumor_grade_se$tumor_grade == "G2"])
normalExp <- rowMeans(logCPM[, tumor_grade_se$tumor_grade == "G4"])
par(mfrow = c(1, 2))
plot(normalExp, tumorExp, xlab = "Tumor", ylab = "Normal", pch = ".", cex = 4, las = 1)
plot((tumorExp + normalExp)/2, tumorExp - normalExp, pch = ".", cex = 4, las = 1)
