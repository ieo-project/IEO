...
setwd("D:/Documents/BIM/3Q/IEO/project")
...
#t
threshold <- 21
depth <- colSums(assays(se)$counts)/1e6 > threshold
se_depth <- se[, depth]
dge_depth <- dge[, depth]
```
...
avgexp <- rowMeans(assays(se_depth)$logCPM)
...
```{r}
#saveRDS(se_exp, file.path("results", "se.unnorm.rds"))
#saveRDS(dge_exp, file.path("results", "dge.filt.unnorm.rds"))
```
...
```{r}
dge_exp <- calcNormFactors(dge_exp)
```
...
```{r}
assays(se_exp)$logCPM <- cpm(dge_exp, log=TRUE, normalized.lib.sizes=TRUE, prior.count=0.25)
```
...
#setmp <- se_exp[, se_exp$type == "tumor"]
#dgetmp <- dge_exp[, se_exp$type == "tumor"]
...
#}
dge_exp$samples$group <- se_exp$type
table(dge_exp$samples$group)
dge2 <- DGEList(counts = assays(se_exp)$counts, genes = as.data.frame(mcols(se_exp)), group = se_exp$type)

plotSmear(dge2, lowess = TRUE, las = 1, cex.lab = 1.5, cex.axis = 1.2)
abline(h = 0, col = "blue", lwd = 2)
